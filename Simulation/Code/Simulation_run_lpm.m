...
for run = 1:25
    ...
    C = cat(3, C1, C2, C3);
    disp = true;
    [C_save, ~, adj_save, Phi_save, ar_Phi] = ...
        MCMC_linked_precision_matrices(S, [n1, n2, n3], C, Phi, ...
        v0, v1, lambda, pii, burnin, nmc, disp);
    
    % Estimated value of Phi (cross-graph correlation measure)
    est_Phi = mean(Phi_save, 3);
    ...
    [C_save_resample, ~] = ...
        MCMC_resample_C_for_fixed_graph(median_model, S, [n1, n2, n3], C, est_Phi, ...
    ...
    cur_file = strcat('perf_iter', num2str(iter), '_n', num2str(n), '_lpm_sim_p100.csv');
    csvwrite(cur_file, {tpr_cur, fpr_cur, mcc_cur, auc_cur, tpr_diff, fpr_diff, ...
        mcc_diff, auc_diff, fl_cur});
    cur_file = strcat('PPI_iter', num2str(iter), '_n', num2str(n), '_lpm_sim_p100.csv');
    csvwrite(cur_file, ppi);
    cur_file = strcat('Phi_', num2str(iter), '_n', num2str(n), '_lpm_sim_p100.csv');
    csvwrite(cur_file, est_Phi);
